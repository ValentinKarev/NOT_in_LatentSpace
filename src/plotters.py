...
from .vae import latents_to_pil
...
def plot_images_from_latent(X, Y, T):
    ...
    with torch.no_grad():
        ...
        for x, y, t_x in zip(X, Y, T_X):
            x = tfms.ToTensor()(latents_to_pil(x[np.newaxis, :, :, :])[0])
            y = tfms.ToTensor()(latents_to_pil(y[np.newaxis, :, :, :])[0])
            t_x = tfms.ToTensor()(latents_to_pil(t_x[np.newaxis, :, :, :])[0])
            
            x_tmp.append(x)
            y_tmp.append(y)
            x_t_tmp.append(t_x)
        
        T_X = torch.stack(x_t_tmp)
        X = torch.stack(x_tmp)
        Y = torch.stack(y_tmp)
        
        #print(X.shape, Y.shape, T_X.shape)
...
def plot_random_images_from_latent(X_sampler, Y_sampler, T):
    X = X_sampler.sample(10)
    Y = Y_sampler.sample(10)
    return plot_images_from_latent(X, Y, T)
...
def plot_Z_images_from_latent(XZ, Y, T):
    freeze(T);
    with torch.no_grad():
        T_XZ = T(
            XZ.flatten(start_dim=0, end_dim=1)
...
def plot_random_Z_images_from_latent(X_sampler, ZC, Z_STD, Y_sampler, T):
    X = X_sampler.sample(10)[:,None].repeat(1,4,1,1,1)
    with torch.no_grad():
        Z = torch.randn(10, 4, ZC, X.size(3), X.size(4), device='cuda') * Z_STD
        XZ = torch.cat([X, Z], dim=2)

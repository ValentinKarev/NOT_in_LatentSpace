...
from .dataset import DatasetFromLatent
from .vae import latent_to_pil_from_bath_of_tensor
...
def load_dataset(name, path, img_size=64, batch_size=64, test_ratio=0.1, device='cuda'):
    if name in ['shoes', 'handbag', 'outdoor', 'church']:
        ...
        #print(len(dataset))
    elif name in ['celeba_female_latent', 'celeba_male_latent']:
        dataset = DatasetFromLatent(path, name.split('_')[-2])
        print(len(dataset))
    elif name in ['celeba_female_tensor', 'celeba_male_tensor']:
    ...
    if name in ['celeba_female', 'celeba_male']:
        with open('/home/jovyan/data/CelebAMask-HQ-attribute-anno.txt', 'r') as f:
            lines = f.readlines()[2:]
        if name == 'celeba_female':
            #idx = [i for i in list(range(len(lines))) if lines[i].replace('  ', ' ').split(' ')[21] == '-1']
            #print(len(lines))
            idx = torch.load('/home/jovyan/NOT_in_latent_space/female.pt').to(int)
        else:
            #idx = [i for i in list(range(len(lines))) if lines[i].replace('  ', ' ').split(' ')[21] != '-1']
            idx = torch.load('/home/jovyan/NOT_in_latent_space/male.pt').to(int)
    ...
    train_sampler = LoaderSampler(DataLoader(train_set, shuffle=True, num_workers=4, batch_size=batch_size, pin_memory=True), device)
    test_sampler = LoaderSampler(DataLoader(test_set, shuffle=True, num_workers=4, batch_size=batch_size,  pin_memory=True), device)
...
def get_pushed_loader_stats_from_latent(T, loader, batch_size=8, verbose=False, device='cuda',
                            use_downloaded_weights=False):
    dims = 2048
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx], use_downloaded_weights=use_downloaded_weights).to(device)
    ...
    with torch.no_grad():
        for step, (X, _) in enumerate(loader) if not verbose else tqdm(enumerate(loader)):
            for i in range(0, len(X), batch_size):
                ...
                batch = latent_to_pil_from_bath_of_tensor(batch)
                
                batch = Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(batch)
...
def get_Z_pushed_loader_stats_from_latent(T, loader, ZC=1, Z_STD=0.1, batch_size=8, verbose=False,
                              device='cuda',
                              use_downloaded_weights=False):
    dims = 2048
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
